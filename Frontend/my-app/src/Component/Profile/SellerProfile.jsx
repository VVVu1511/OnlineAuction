...
export default function SellerProfile({ user, token }) {
    ...
    async function loadData() {
        setLoading(true);
        try {
        // endpoints are examples; chỉnh lại theo backend của bạn
        
        const [myRes, wonRes] = await Promise.all([
            fetch('http://localhost:3000/api/seller/my-products', { headers }),
            fetch('http://localhost:3000/api/seller/won-products', { headers }),
        ]);
    ...
    async function submitAppend() {
        ...
        try {
        const res = await fetch(`/api/product/${currentProduct.id}/append-description`, {
            method: 'POST',
            headers,
